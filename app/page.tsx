...
function drawLogo(ctx: CanvasRenderingContext2D, logo: HTMLImageElement | null, x: number, y: number, width: number, height: number, scalePercent: number, opacity: number, position: LogoPosition = "center") {
  ...
  const logoWidth = logo.naturalWidth * scale, logoHeight = logo.naturalHeight * scale;
  const pad = Math.max(6, Math.min(width, height) * 0.06);
  const horizontal = position.endsWith("left") ? x + pad : position.endsWith("right") ? x + width - logoWidth - pad : x + (width - logoWidth) / 2;
  const vertical = position.startsWith("top") ? y + pad : position.startsWith("bottom") ? y + height - logoHeight - pad : y + (height - logoHeight) / 2;
  ctx.save(); ctx.globalAlpha = opacity / 100; ctx.drawImage(logo, horizontal, vertical, logoWidth, logoHeight); ctx.restore();
}
...
function drawMetric(ctx: CanvasRenderingContext2D, width: number, height: number, config: PatternConfig, logo: HTMLImageElement | null) {
  ...
  if (config.showDiagonals) { line(ctx, 0, 0, width, height, config.diagonalColor, major, 0.85); line(ctx, width, 0, 0, height, config.diagonalColor, major, 0.85); }
...
function drawBasicPattern(ctx: CanvasRenderingContext2D, width: number, height: number, type: PatternType) {
  ...
  else { const image = ctx.createImageData(width, height); for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) { const offset = (y * width + x) * 4, v = (x + y) % 2 ? 0 : 255; image.data[offset] = v; image.data[offset + 1] = v; image.data[offset + 2] = v; image.data[offset + 3] = 255; } ctx.putImageData(image, 0, 0); }
}
...
function drawSliceInformation(ctx: CanvasRenderingContext2D, slice: ResolumeSlice, rect: Rect, settings: PatternConfig & SliceOverride) {
  ...
  const rawItems = allItems.filter((item) => item.position !== "hidden");
  const positions = Array.from(new Set(rawItems.map((item) => item.position))) as Array<Exclude<InfoPosition, "hidden">>;
  positions.forEach((position) => {
    ...
    const groupWidth = Math.max(...measured.map((item) => item.width)), groupHeight = measured.reduce((sum, item) => sum + item.height, 0), angle = settings.infoOrientation === "rotate-90" ? Math.PI / 2 : settings.infoOrientation === "rotate-180" ? Math.PI : settings.infoOrientation === "rotate-270" ? -Math.PI / 2 : 0, quarterTurn = Math.abs(angle) === Math.PI / 2, boundsWidth = quarterTurn ? groupHeight : groupWidth, boundsHeight = quarterTurn ? groupWidth : groupHeight, anchor = positionAnchor(rect, position, boundsWidth, boundsHeight);
    ctx.save(); ctx.translate(anchor.x, anchor.y); ctx.rotate(angle); let cursorY = -groupHeight / 2;
    measured.forEach((item, index) => { const left = anchor.horizontal === "left" ? -groupWidth / 2 : anchor.horizontal === "right" ? groupWidth / 2 - item.width : -item.width / 2, dark = index % 2 === 0; ctx.fillStyle = dark ? "#050505" : "#ffffff"; ctx.fillRect(left, cursorY, item.width, item.height); ctx.fillStyle = dark ? "#ffffff" : "#050505"; ctx.font = `${item.weight} ${item.fontSize}px ${item.family}`; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(item.text, left + item.width / 2, cursorY + item.height / 2); cursorY += item.height; });
    ctx.restore();
  });
}
...
function drawPixelMap(ctx: CanvasRenderingContext2D, width: number, height: number, slices: ResolumeSlice[], view: MapView, config: PatternConfig, logo: HTMLImageElement | null, overrides: Record<string, SliceOverride>, selectedIds: string[] = [], showSelection = false) {
  if (config.backgroundMode === "black") { ctx.fillStyle = "#000"; ctx.fillRect(0, 0, width, height); } else ctx.clearRect(0, 0, width, height);
  slices.forEach((slice) => {
    ...
    if (settings.showDiagonals) { line(ctx, rect.x, rect.y, rect.x + rect.width, rect.y + rect.height, settings.diagonalColor, stroke, 0.62); line(ctx, rect.x + rect.width, rect.y, rect.x, rect.y + rect.height, settings.diagonalColor, stroke, 0.62); }
    if (settings.showCircles) { ctx.strokeStyle = settings.circleColor; ctx.globalAlpha = 0.62; ctx.lineWidth = stroke; ctx.beginPath(); ctx.ellipse(rect.x + rect.width / 2, rect.y + rect.height / 2, Math.min(rect.width * 0.34, rect.height * 0.44), Math.min(rect.width * 0.34, rect.height * 0.44), 0, 0, Math.PI * 2); ctx.stroke(); }
    if (settings.showSafeArea) { ctx.strokeStyle = settings.safeAreaColor; ctx.globalAlpha = 0.78; ctx.lineWidth = stroke; ctx.setLineDash([stroke * 8, stroke * 6]); ctx.strokeRect(rect.x + rect.width * 0.05, rect.y + rect.height * 0.05, rect.width * 0.9, rect.height * 0.9); ctx.setLineDash([]); }
    if (settings.showCenterDot) { ctx.fillStyle = settings.centerDotColor; ctx.globalAlpha = 1; ctx.beginPath(); ctx.arc(rect.x + rect.width / 2, rect.y + rect.height / 2, Math.max(2, settings.centerDotSize / 2), 0, Math.PI * 2); ctx.fill(); }
    if (override.logoVisible ?? config.showLogo) drawLogo(ctx, logo, rect.x, rect.y, rect.width, rect.height, override.logoScale ?? config.customLogoScale, config.customLogoOpacity, override.logoPosition ?? config.customLogoPosition);
    drawSliceInformation(ctx, slice, rect, settings);
    ctx.restore(); ctx.save(); ctx.strokeStyle = settings.metricGridColor; ctx.lineWidth = Math.max(1, settings.lineWidth * width / 5760); ctx.beginPath(); rect.points.forEach((point, index) => index ? ctx.lineTo(point.x, point.y) : ctx.moveTo(point.x, point.y)); ctx.closePath(); ctx.stroke(); ctx.restore();
...
export default function Home() {
  ...
  const [resolumeMap, setResolumeMap] = useState<ResolumeMap | null>(null), [rawXml, setRawXml] = useState(""), [xmlName, setXmlName] = useState(""), [xmlError, setXmlError] = useState(""), [selectedScreen, setSelectedScreen] = useState(0);
  ...
  const [zoom, setZoom] = useState(1), [pan, setPan] = useState({ x: 0, y: 0 }), [spaceDown, setSpaceDown] = useState(false), [stageBounds, setStageBounds] = useState({ width: 1, height: 1 }); const dragRef = useRef<{ x: number; y: number; panX: number; panY: number; moved: boolean } | null>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null), canvasStageRef = useRef<HTMLDivElement>(null), fullscreenHostRef = useRef<HTMLDivElement>(null), xmlInputRef = useRef<HTMLInputElement>(null), logoInputRef = useRef<HTMLInputElement>(null), projectInputRef = useRef<HTMLInputElement>(null);
  ...
  const renderToCanvas = useCallback((canvas: HTMLCanvasElement, mode = workspaceMode, view = mapView, screen = activeScreen, slices?: ResolumeSlice[], selection = true) => {
    ...
    canvas.width = clamp(Math.round(width), 1, 16384); canvas.height = clamp(Math.round(height), 1, 16384); const ctx = canvas.getContext("2d", { alpha: true }); if (!ctx) return;
    if (mode === "resolume" && resolumeMap) drawPixelMap(ctx, canvas.width, canvas.height, slices || (view === "input" ? allSlices : screen?.slices || []), view, config, logoImage, sliceOverrides, selectedSliceIds, selection);
    else if (config.pattern === "metric") drawMetric(ctx, canvas.width, canvas.height, config, logoImage); else if (config.pattern === "cabinet") drawCabinets(ctx, canvas.width, canvas.height, config); else drawBasicPattern(ctx, canvas.width, canvas.height, config.pattern);
  }, [activeScreen, allSlices, config, logoImage, mapView, resolumeMap, selectedSliceIds, sliceOverrides, workspaceMode]);
  useEffect(() => { if (canvasRef.current) renderToCanvas(canvasRef.current); }, [renderToCanvas]);
  ...
  const loadXml = useCallback((file?: File) => { if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const xml = String(reader.result), map = parseResolumeXml(xml); setResolumeMap(map); setRawXml(xml); setXmlName(file.name); setXmlError(""); setSelectedScreen(0); setSelectedSliceIds([]); setWorkspaceMode("resolume"); setZoom(1); setPan({ x: 0, y: 0 }); setNotice(`Loaded ${map.screens.length} screens and ${map.screens.flatMap((screen) => screen.slices).length} slices`); } catch (error) { setXmlError(error instanceof Error ? error.message : "Unable to read this XML file."); } }; reader.readAsText(file); }, []);
  ...
  const selectCanvasSlice = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => { if (workspaceMode !== "resolume" || dragRef.current?.moved) return; const canvas = event.currentTarget, box = canvas.getBoundingClientRect(), x = (event.clientX - box.left) * canvas.width / box.width, y = (event.clientY - box.top) * canvas.height / box.height, hit = [...activeSlices].reverse().find((slice) => { const rect = mapView === "input" ? slice.input : slice.output; return x >= rect.x && x <= rect.x + rect.width && y >= rect.y && y <= rect.y + rect.height; }); if (!hit) { if (!event.ctrlKey && !event.shiftKey) setSelectedSliceIds([]); return; } if (event.ctrlKey || event.shiftKey) setSelectedSliceIds((current) => current.includes(hit.id) ? current.filter((id) => id !== hit.id) : [...current, hit.id]); else setSelectedSliceIds([hit.id]); }, [activeSlices, mapView, workspaceMode]);
  const beginPan = (event: React.PointerEvent<HTMLDivElement>) => { if (event.button === 1 || (event.button === 0 && spaceDown)) { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); dragRef.current = { x: event.clientX, y: event.clientY, panX: pan.x, panY: pan.y, moved: false }; } };
  const movePan = (event: React.PointerEvent<HTMLDivElement>) => { if (!dragRef.current) return; const dx = event.clientX - dragRef.current.x, dy = event.clientY - dragRef.current.y; if (Math.abs(dx) + Math.abs(dy) > 3) dragRef.current.moved = true; setPan({ x: dragRef.current.panX + dx, y: dragRef.current.panY + dy }); };
  const endPan = () => { window.setTimeout(() => { dragRef.current = null; }, 0); };
  const adjustZoom = (next: number, clientX?: number, clientY?: number) => {
    const target = clamp(round(next, 2), 0.1, 8), stage = canvasStageRef.current;
    if (stage && clientX !== undefined && clientY !== undefined) {
      const box = stage.getBoundingClientRect(), pointerX = clientX - box.left - box.width / 2, pointerY = clientY - box.top - box.height / 2, ratio = target / zoom;
      setPan({ x: pointerX - (pointerX - pan.x) * ratio, y: pointerY - (pointerY - pan.y) * ratio });
    }
    setZoom(target);
  };
  const resetView = () => { setZoom(1); setPan({ x: 0, y: 0 }); setFullscreenMode("fit"); };
    ...
    <section className="workspace">
      <aside className="left-panel panel"><div className="panel-tabs" data-count={controlTabs.length}>{controlTabs.map((tab) => <button key={tab} className={controlTab === tab ? "active" : ""} onClick={() => setControlTab(tab)}>{workspaceMode === "resolume" && tab === "setup" ? "Source" : tab}</button>)}</div><div className="panel-content">
        ...
        {controlTab === "setup" && workspaceMode === "resolume" && <>
          <section className="compact-section"><span className="eyebrow">Advanced Output XML</span><button className="drop-button" onClick={() => xmlInputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); loadXml(event.dataTransfer.files?.[0]); }}><strong>Choose XML</strong><small>or drag a Resolume preset here</small></button><input ref={xmlInputRef} hidden type="file" accept=".xml,text/xml" onChange={(event) => loadXml(event.target.files?.[0])} />{xmlName && <div className="file-status"><i /><span>{xmlName}</span></div>}{xmlError && <p className="warning">{xmlError}</p>}</section>
        ...
        {controlTab === "logo" && <section className="compact-section logo-controls">
          ...
          <button className={(selectedOverride.logoVisible ?? config.showLogo) ? "visibility-button active" : "visibility-button"} onClick={() => { const visible = selectedOverride.logoVisible ?? config.showLogo; selectedSliceIds.length ? updateSelected({ logoVisible: !visible }) : updateGlobal("showLogo", !visible); }}>{(selectedOverride.logoVisible ?? config.showLogo) ? (selectedSliceIds.length ? "Logo visible on selection" : "Logo visible globally") : (selectedSliceIds.length ? "Logo hidden on selection" : "Logo hidden globally")}</button>
      ...
      <section className="stage">
        ...
        <div className="preview-shell" ref={fullscreenHostRef} data-fullscreen-mode={fullscreenMode}>
          <div className="preview-toolbar"><span><i className="green" /> Pixel canvas · {outputWidth} × {outputHeight}</span><div className="view-tools"><button onClick={() => adjustZoom(zoom / 1.2)} aria-label="Zoom out">−</button><output>{Math.round(displayScale * 100)}%</output><button onClick={() => adjustZoom(zoom * 1.2)} aria-label="Zoom in">+</button><button className={fullscreenMode === "fit" && zoom === 1 ? "active" : ""} onClick={resetView}>Fit Canvas</button><button className={fullscreenMode === "actual" && zoom === 1 ? "active" : ""} onClick={() => { setFullscreenMode("actual"); setZoom(1); setPan({ x: 0, y: 0 }); }}>Actual 1:1</button></div></div>
          <div ref={canvasStageRef} className={`canvas-stage ${spaceDown ? "panning" : ""}`} onPointerDown={beginPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan} onWheel={(event) => { event.preventDefault(); adjustZoom(zoom * (event.deltaY > 0 ? 0.9 : 1.1), event.clientX, event.clientY); }}><canvas ref={canvasRef} onClick={selectCanvasSlice} style={{ width: `${outputWidth * baseScale}px`, height: `${outputHeight * baseScale}px`, transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }} aria-label="Live LED pattern output" /></div>
        </div>
