...
function ExpressionField({ label, value, suffix, onCommit, integer = false, min = integer ? 1 : 0.0001, max = Number.POSITIVE_INFINITY }: { label: string; value: number; suffix: string; onCommit: (value: number) => void; integer?: boolean; min?: number; max?: number }) {
  ...
  const adjust = (direction: 1 | -1) => { const parsed = evaluateExpression(draft) ?? value, increment = integer ? 1 : 0.1, next = clamp(integer ? Math.round(parsed + direction) : round(parsed + direction * increment, 4), min, max); setInvalid(false); setDraft(String(next)); onCommit(next); };
...
function PreciseNumberInput({ label, value, min, max, step = 1, disabled = false, onEditStart, onChange }: { label: string; value: number; min: number; max: number; step?: number; disabled?: boolean; onEditStart?: () => void; onChange: (value: number) => void }) {
  ...
  return <span className="precise-stepper"><input disabled={disabled} aria-label={label} className="precise-input" type="number" min={min} max={max} step={step} value={value} onFocus={onEditStart} onChange={(event) => apply(Number(event.target.value))} /><span className="field-stepper"><button type="button" tabIndex={-1} disabled={disabled} aria-label={`Increase ${label}`} onPointerDown={(event) => event.preventDefault()} onClick={() => { onEditStart?.(); apply(value + step); }}>▲</button><button type="button" tabIndex={-1} disabled={disabled} aria-label={`Decrease ${label}`} onPointerDown={(event) => event.preventDefault()} onClick={() => { onEditStart?.(); apply(value - step); }}>▼</button></span></span>;
}
...
export default function Home() {
  ...
  return <TransformSelectionScope.Provider value={selectedSliceIds.join("|")}><main className="app-shell">
    ...
    </section>{notice && <button className="toast" onClick={() => setNotice("")}>{notice}<span>×</span></button>}{availableUpdate && <div className="toast update-toast" role="status"><div><strong>Pattern Lab {availableUpdate.latestVersion} is available</strong><small>{availableUpdate.name || "A newer desktop release is ready."}</small></div><div className="update-actions"><button onClick={() => void (window as PickerWindow).lo2sDesktop?.openExternal(availableUpdate.url || "https://github.com/johnjjdave/lo2s-pattern-lab/releases/latest")}>View changes</button><button className="primary" onClick={() => void (window as PickerWindow).lo2sDesktop?.openExternal(availableUpdate.url || "https://github.com/johnjjdave/lo2s-pattern-lab/releases/latest")}>Download</button><button className="dismiss" aria-label="Remind me in seven days" onClick={() => { if (availableUpdate.latestVersion) localStorage.setItem(`lo2s-update-dismissed:${availableUpdate.latestVersion}`, String(Date.now())); setAvailableUpdate(null); }}>×</button></div></div>}
